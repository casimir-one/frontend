...
describe('deip.api:', function () {
  ...
  describe('getFollowers', () => {
    describe('getting ned\'s followers', () => {
      it('works', async () => {
        const result = await deipRpc.api.getFollowersAsync('ned', 0, 'blog', 5);
      ...
      it('the startFollower parameter has an impact on the result', async () => {
        // Get the first 5
        const result1 = await deipRpc.api.getFollowersAsync('ned', 0, 'blog', 5)
          result1.should.have.lengthOf(5);
        const result2 = await deipRpc.api.getFollowersAsync('ned', result1[result1.length - 1].follower, 'blog', 5)
          result2.should.have.lengthOf(5);
        result1.should.not.be.eql(result2);
      });

      it('clears listeners', async () => {
        deipRpc.api.listeners('message').should.have.lengthOf(0);
      });
    });
  });
