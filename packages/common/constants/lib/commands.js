import { createEnum } from '@deip/toolbox';

const APP_CMD = createEnum({
  CREATE_DAO: 1,
  UPDATE_DAO: 2,
  CREATE_PROJECT: 3,
  UPDATE_PROJECT: 4,
  DELETE_PROJECT: 5,
  ADD_DAO_MEMBER: 6,
  CREATE_PROPOSAL: 7,
  ACCEPT_PROPOSAL: 8,
  DECLINE_PROPOSAL: 9,
  CREATE_ATTRIBUTE: 10,
  UPDATE_ATTRIBUTE: 11,
  DELETE_ATTRIBUTE: 12,
  REMOVE_DAO_MEMBER: 13,
  CREATE_INVESTMENT_OPPORTUNITY: 14,
  INVEST: 15,
  TRANSFER_FT: 16,
  TRANSFER_NFT: 17,
  CREATE_DOCUMENT_TEMPLATE: 18,
  UPDATE_DOCUMENT_TEMPLATE: 19,
  DELETE_DOCUMENT_TEMPLATE: 20,
  CREATE_FT: 21,
  CREATE_NFT: 22,
  ISSUE_FT: 23,
  ISSUE_NFT: 24,
  CREATE_DRAFT: 25,
  UPDATE_DRAFT: 26,
  DELETE_DRAFT: 27,
  CREATE_PROJECT_CONTENT: 28,
  CREATE_REVIEW_REQUEST: 29,
  DECLINE_REVIEW_REQUEST: 30,
  CREATE_REVIEW: 31,
  UPVOTE_REVIEW: 32,
  CREATE_PROJECT_NDA: 33,
  CREATE_CONTRACT_AGREEMENT: 34,
  ACCEPT_CONTRACT_AGREEMENT: 35,
  REJECT_CONTRACT_AGREEMENT: 36,
  UPDATE_PORTAL_PROFILE: 37,
  UPDATE_PORTAL_SETTINGS: 38,
  CREATE_LAYOUT: 39,
  UPDATE_LAYOUT: 40,
  DELETE_LAYOUT: 41,
  UPDATE_LAYOUT_SETTINGS: 42,
  UPDATE_ATTRIBUTE_SETTINGS: 43,
  UPDATE_NETWORK_SETTINGS: 44,
  DELETE_USER_PROFILE: 45,
  ALTER_DAO_AUTHORITY: 46,
  CREATE_BOOKMARK: 47,
  DELETE_BOOKMARK: 48,
  MARK_NOTIFICATIONS_AS_READ: 49,
  CREATE_PORTAL: 50
});

const APP_PROPOSAL = createEnum({
  PROJECT_PROPOSAL: 1,
  PROJECT_UPDATE_PROPOSAL: 2,
  PROJECT_CONTENT_PROPOSAL: 3,
  PROJECT_FUNDRASE_PROPOSAL: 4,
  TEAM_UPDATE_PROPOSAL: 5,
  ADD_DAO_MEMBER_PROPOSAL: 6,
  REMOVE_DAO_MEMBER_PROPOSAL: 7,
  FT_TRANSFER_PROPOSAL: 8,
  NFT_TRANSFER_PROPOSAL: 9,
  EXPRESS_LICENSE_PROPOSAL: 10,
  TOKENS_SWAP_PROPOSAL: 11,
  PROJECT_NDA_PROPOSAL: 12,
  INVESTMENT_OPPORTUNITY_PROPOSAL: 13,
  CONTRACT_AGREEMENT_PROPOSAL: 14
});

export {
  APP_CMD,
  APP_PROPOSAL
};
