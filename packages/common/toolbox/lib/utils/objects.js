...
export const sortObjectKeys = (obj, comparator) => {
  ...
  if (isObject(clone)) {
    const keys = sortBy(Object.keys(clone), key => comparator ? comparator(obj[key], key) : key);
...
export const camelizeObjectKeys = (obj, exception = ['_id', '__v']) => {
  if (!obj) return {};
  return Object.keys(obj).reduce((o, key) => ({ ...o,
    ...{
...
export const genObjectId = (obj, turns = 3) => {
  ...
  return new Array(turns).fill(null).map((x, index) => index).reduce((x, index) => x + crc32(`turn-${index + 1}-${JSON.stringify(sorted)}`).toString(16), '');
};
 ...
 * @return {assert}
 */
const deepFindAssert = (condition, failureMessage) => {
  assert(condition, `[deepFindParentByValue]: ${failureMessage}`);
};
