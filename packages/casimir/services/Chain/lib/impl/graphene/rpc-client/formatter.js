...
module.exports = function (deipAPI) {
    ...
    function vestingDeip(account, gprops) {
        ...
        var total_vest_deip = parseFloat(gprops.total_vesting_fund_deip.split(" ")[0]);
        var vesting_deipf = total_vest_deip * (vests / total_vests);
    ...
    function processOrders(open_orders, assetPrecision) {
        ...
        var deipOrders = !open_orders ? 0 : open_orders.reduce(function (o, order) {
        ...
        return { deipOrders: deipOrders, sbdOrders: sbdOrders };
    }
    ...
    function calculateSaving(savings_withdraws) {
        ...
        savings_withdraws.forEach(function (withdraw) {
            ...
            if (asset === "DEIP") savings_pending += parseFloat(amount);else {
    ...
    function estimateAccountValue(account) {
        var _ref = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : {},
            ...
            vesting_deip = _ref.vesting_deip;
        ...
        if (!vesting_deip || !feed_price) {
            if (!gprops || !feed_price) {
                promises.push(deipAPI.getStateAsync("/@{username}").then(function (data) {
                    gprops = data.props;
        ...
        if (!open_orders) {
            promises.push(deipAPI.getOpenOrdersAsync(username).then(function (open_orders) {
        ...
        if (!savings_withdraws) {
            promises.push(deipAPI.getSavingsWithdrawFromAsync(username).then(function (savings_withdraws) {
        ...
        return Promise.all(promises).then(function () {
            var price_per_deip = undefined;
            var _feed_price = feed_price,
                base = _feed_price.base,
                quote = _feed_price.quote;

            if (/ SBD$/.test(base) && / DEIP$/.test(quote)) price_per_deip = parseFloat(base.split(" ")[0]);
            var savings_balance = account.savings_balance;
            var savings_sbd_balance = account.savings_sbd_balance;
            var balance_deip = parseFloat(account.balance.split(" ")[0]);
            var saving_balance_deip = parseFloat(savings_balance.split(" ")[0]);
            ...
            var total_deip = vesting_deip + balance_deip + saving_balance_deip + savings.savings_pending + orders.deipOrders;
    ...
    return {
        ...
        vestToDeip: function vestToDeip(vestingShares, totalVestingShares, totalVestingFundDeip) {
            return parseFloat(totalVestingFundDeip) * (parseFloat(vestingShares) / parseFloat(totalVestingShares));
        },
