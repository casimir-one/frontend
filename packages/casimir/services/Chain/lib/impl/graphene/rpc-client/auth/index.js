...
Auth.signTransaction = function (trx, keys, tenantStamp) {
    ...
    if (trx.tenant_signature) {
        ...
        var tenant = tenantStamp.tenant;
        var tenantPrivKey = tenantStamp.tenantPrivKey;
        var chainId = new Buffer(config.get('chain_id'), 'hex');
        var txBuffer = transaction.toBuffer(trx);
        var tenantSig = Signature.signBuffer(Buffer.concat([chainId, txBuffer]), tenantPrivKey);
        tenant_signature = {
            tenant: tenant,
