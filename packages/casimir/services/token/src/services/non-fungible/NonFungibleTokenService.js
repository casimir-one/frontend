...
export class NonFungibleTokenService {
  ...
  async createNftItem(payload) {
    ...
    const createNftItemMetadataCmd = new CreateNftItemMetadataCmd({
      ...
      issuer,
      contentType,
      description: genSha256Hash({ nftItem: { title } }),
      nftItemMetadataDraftId,
      authors,
      metadata,
      references,
  ...
  async getPublicNftCollectionsList(filter) {
    ...
    return this.nonFungibleTokenHttp.getPublicNftCollectionsList({
      attributes,
      portalIds
    });
  }
  ...
  async getPublicNftItemsList() {
    return this.nonFungibleTokenHttp.getPublicNftItemsList();
  }
