<template>
  <div>
    ...
    <v-list>
      <v-list-item v-for="file in nftItem.packageFiles" :key="file.hash">
        <v-list-item-content>
          <v-row>
            <v-col cols="10">
            ...
            <v-col class="text-align-right">
              <vex-tooltip
                :tooltip="$t('module.nftItems.details.download')"
                bottom
              >
                <v-btn
                  icon
  ...
  export default defineComponent({
    ...
    methods: {
      ...
      getContentUrl(fileHash, download = false) {
        ...
        return `${DEIP_SERVER_URL}/api/v2/nft-items/package/${this.content._id}/`
          + `${fileHash}?download=${download}&authorization=${accessService.getAccessToken()}`;
      }
    }
  });
</script>
